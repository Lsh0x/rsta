...
///
/// Returns `(index, value)` pairs so callers can align with the source data.
...
/// Compare a computed series `produced` (values aligned to the *end* of the
/// source data — i.e. with `n_input - len(produced)` warmup rows missing at
/// the head) against a `(index, value)` golden series, using `tol` as absolute
/// tolerance.
